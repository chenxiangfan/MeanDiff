...
from models import ns_Transformer
...
def create_model_and_diffusion(cfg):
    ...
    GCN_model = ns_Transformer.Model().to(cfg.device)

    return model, diffusion, GCN_model
...
def sample_preprocessing(traj, cfg, mode):
    ...
    if mode.split('_')[0] == 'fix':
        ...
        # his_mean = torch.mean(traj[:, :cfg.t_his, :], dim=1).unsqueeze(1)
        # traj_his_zero = traj - his_mean
        # traj_his_pad = padding_traj(traj_his_zero, cfg.padding, cfg.idx_pad, cfg.zero_index)
        # traj_pad = padding_traj(traj, cfg.padding, cfg.idx_pad, cfg.zero_index)
        #
        # traj_his_dct = torch.matmul(cfg.dct_m_all[:cfg.n_pre], traj_his_pad)
        # traj_dct = torch.matmul(cfg.dct_m_all[:cfg.n_pre], traj_pad)
        # traj_dct_mod = copy.deepcopy(traj_dct)
        # if np.random.random() > cfg.mod_test:
        #     traj_dct_mod = None
