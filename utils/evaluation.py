...
def compute_stats(diffusion, multimodal_dict, model, GCN_model, logger, cfg):
    """
    ...
    def get_prediction(data, model_select, GCN_model):
        ...
        tarj_his = traj[:, :cfg.t_his, :]
        traj_his_dct = torch.matmul(cfg.dct_m_all[:cfg.n_pre, :cfg.t_his], tarj_his)
    ...
    for i in tqdm(range(0, K), position=0):
        ...
        pred_i_nd = get_prediction(data_group, model, GCN_model)
        ...
        if i == K - 1:  # in last iteration, concatenate all candidate pred
            ...
            gt_group = torch.from_numpy(gt_group).to(cfg.device)
