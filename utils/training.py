...
class Trainer:
    ...
    def run_train_step(self):
        ...
        for traj_np in self.generator_train:
            with torch.no_grad():
                ...
                # his_mean = torch.mean(traj[:, :self.cfg.t_his, :], dim=1).unsqueeze(1)
                tarj_his = traj[:, :self.cfg.t_his, :]
                tarj_his_dct = torch.matmul(self.cfg.dct_m_all[:self.cfg.n_pre, :self.cfg.t_his], tarj_his)
            ...
            # train
            # print('xxxxxxxxxxxxxxxxx', traj_dct_mod.size())
            # print('xxxxxxxxxxxxxxxxx', traj_dct_mod)
            # exit(0)
            out_gcn = self.GCN_model(tarj_his_dct, traj_dct_mod)
            # his_std = torch.std(traj_his, dim=1).unsqueeze(1)
            # his_std = his_std.repeat(1, traj.shape[1], 1).to(self.cfg.device)
            out_mean = torch.mean(out_gcn, dim=1).unsqueeze(1)

            t = self.diffusion.sample_timesteps(traj.shape[0]).to(self.cfg.device)
            x_t, noise = self.diffusion.noise_motion(traj_dct, traj, t, out_mean)
            if np.random.random() > self.cfg.mod_train:
                traj_dct_mod = None

            predicted_noise = self.model(x_t, t, mod=traj_dct_mod)

            loss = self.criterion(predicted_noise, noise) + self.criterion(out_mean,
                                                                           torch.mean(traj_dct, dim=1).unsqueeze(1))
    ...
    def run_val_step(self):
        for traj_np in self.generator_val:
            with torch.no_grad():
                ...
                loss = self.criterion(predicted_noise, noise) + self.criterion(out_mean,
                                                                               torch.mean(traj_dct, dim=1).unsqueeze(1))
                # loss = self.criterion(predicted_noise, noise) + self.criterion(out_gcn, traj_dct)
                # loss = self.criterion(predicted_noise, noise) + self.criterion(out_gcn.transpose(2, 1), traj_dct)
    ...
    def after_val_step(self):
        ...
        if self.iter % self.cfg.save_metrics_interval == 0 and self.iter != 0:
            if self.cfg.ema is True:
                compute_stats(self.diffusion, self.multimodal_dict, self.ema_model, self.GCN_model, self.logger, self.cfg)
            else:
                compute_stats(self.diffusion, self.multimodal_dict, self.model, self.GCN_model, self.logger, self.cfg)
        if self.cfg.save_model_interval > 0 and (self.iter + 1) % self.cfg.save_model_interval == 0:
            if self.cfg.ema is True:
                ...
                torch.save(self.GCN_model.state_dict(),
                ...
                torch.save(self.GCN_model.state_dict(), os.path.join(self.cfg.model_path, f"ckpt_gcn_{self.iter + 1}.pt"))