...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str,
                        default=torch.device('cuda', index=0) if torch.cuda.is_available() else torch.device('cpu'))
    ...
    parser.add_argument('--save_metrics_interval', type=int, default=1000)
    ...
    """model"""
    model, diffusion, GCN_model = create_model_and_diffusion(cfg)
    ...
    if args.mode == 'train':
        ...
        ckpt = torch.load('/home/fcx/HMP/nsformer_gp/results/h36m_3/models/ckpt_ema_1000.pt')
        gcn_ckpt = torch.load('/home/fcx/HMP/nsformer_gp/results/h36m_3/models/ckpt_gcn_1000.pt')
        model.load_state_dict(ckpt)
        GCN_model.load_state_dict(gcn_ckpt)
        # prepare full evaluation dataset
        multimodal_dict = get_multimodal_gt_full(logger, dataset_multi_test, args, cfg)
        compute_stats(diffusion, multimodal_dict, model, GCN_model, logger, cfg)
    else:
